...
public class PermissionCommand extends Command implements TabExecutor {
    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        if (!logPlayer.hasPermission(PermissionShortCut.PERMISSION_GRANT_COMMAND)) {
            p.sendMessage(CommandPrefix.getNetworkPrefix()
                    + LanguageManager.getMessage(logPlayer.language(), "NoPermission1"));
            return;
        }

        if (args.length <= 2) {
            p.sendMessage(CommandPrefix.getNetworkPrefix()
                    + LanguageManager.getMessage(logPlayer.language(), "ErrorPermissionNEArgs"));
            return;
        }

        if (!PermissionShortCut.isPermission(args[2])) {
            p.sendMessage(CommandPrefix.getNetworkPrefix()
                    + LanguageManager.getMessage(logPlayer.language(), "ErrorNotPSCAvailable"));
            return;
        }
        ...
        if (args[0].equalsIgnoreCase("grant")) {
            if (targetUser != null && targetUser.getCachedData().getPermissionData()
                    .checkPermission(pmSc.getPermission()).asBoolean()) {
                p.sendMessage(CommandPrefix.getNetworkPrefix()
                        + LanguageManager.getMessage(logPlayer.language(), "ErrorHasPermission"));
                return;
            }
            ...
            p.sendMessage(CommandPrefix.getNetworkPrefix()
                    + LanguageManager.getMessage(logPlayer.language(), "PermissionGranted")
                            .replace("{PERMISSION}", pmSc.getPermission()).replace("{PLAYER}", target.getName()));
        } else if (args[0].equalsIgnoreCase("revoke")) {
            if (!(targetUser != null && targetUser.getCachedData().getPermissionData()
                    .checkPermission(pmSc.getPermission()).asBoolean())) {
                p.sendMessage(CommandPrefix.getNetworkPrefix()
                        + LanguageManager.getMessage(logPlayer.language(), "ErrorHasNotPermission"));
                return;
            }
            ...
            userManager.saveUser(targetUser);
            p.sendMessage(CommandPrefix.getNetworkPrefix()
                    + LanguageManager.getMessage(logPlayer.language(), "PermissionRevoked")
                            .replace("{PERMISSION}", pmSc.getPermission()).replace("{PLAYER}", target.getName()));
        }
    }
    ...
    public Iterable<String> onTabComplete(CommandSender sender, String[] args) {
        if (args.length == 1)
            return Arrays.asList("revoke", "grant");
        else if (args.length == 2)
            return SnorlaxLOG.getInstance().getProxy().getPlayers().stream().map(ProxiedPlayer::getName)
                    .collect(Collectors.toList());
        else if (args.length == 3)
            return Arrays.stream(PermissionShortCut.values()).map(PermissionShortCut::getPermission)
                    .collect(Collectors.toList());
        return null;
    }
}
